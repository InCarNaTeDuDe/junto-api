...
async function main() {
  try {
    ...
    console.log(
      "Successfully configured react-native-css-interop cache for CommonJS",
    );

    if (!fs.existsSync(sourceJpg)) {
      console.log(
        `Source image ${sourceJpg} not found. Existing assets in ./assets will be preserved.`,
      );
      return;
    }
    ...
    console.log(
      "All branding assets generated successfully and are completely uncorrupted!",
    );
  } catch (err) {
